...
use std::ops::Deref;
use transformations::LocalTransformationConfig;

#[cfg(test)]
use mockall::*;
#[cfg(test)]
use transformations::LocalTransform;
...
impl FilterConfig {
    ...
    pub fn new(filter_config: &str) -> Option<Self> {
        ...
        Some(FilterConfig{transformations: config})
    }
}
    ...
    fn test_injected_functions() {
        ...
        let mut filter_conf = FilterConfig {
            ...
            response: LocalTransform {
                set: vec![("X-Bar".to_string(), "foo".to_string())],
            }
        };
