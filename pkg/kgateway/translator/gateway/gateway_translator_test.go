...
func TestBasic(t *testing.T) {
	...
	t.Run("HttpACL Policy at route level", func(t *testing.T) {
		test(t, translatorTestCase{
			inputFile:  "httpacl/route-http-acl.yaml",
			outputFile: "httpacl/route-http-acl.yaml",
			gwNN: types.NamespacedName{
				Namespace: "default",
				Name:      "example-gateway",
			},
	...
	t.Run("HttpACL Policy at httproute level", func(t *testing.T) {
		test(t, translatorTestCase{
			inputFile:  "httpacl/httproute-http-acl.yaml",
			outputFile: "httpacl/httproute-http-acl.yaml",
			gwNN: types.NamespacedName{
				Namespace: "default",
				Name:      "example-gateway",
			},
	...
	t.Run("HttpACL Policy at gateway level", func(t *testing.T) {
		test(t, translatorTestCase{
			inputFile:  "httpacl/gateway-http-acl.yaml",
			outputFile: "httpacl/gateway-http-acl.yaml",
			gwNN: types.NamespacedName{
				Namespace: "default",
				Name:      "example-gateway",
			},
	...
	t.Run("HttpACL Policy with invalid CIDR rejected by schema validation", func(t *testing.T) {
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()
		dir := fsutils.MustGetThisDir()

		tc := translatortest.TestCase{
			InputFiles: []string{filepath.Join(dir, "testutils/inputs/httpacl/gateway-http-acl-invalid-cidr.yaml")},
		}
